...
(() => {
  ...
  const openBtn = document.getElementById("themePopupOpen");
  const closeBtn = document.getElementById("themePopupClose");
  const toggleBtn = document.getElementById("popupThemeToggle");
  const textEl = document.getElementById("popupThemeText");
  const iconEl = document.getElementById("popupThemeIcon");

  if (!popup || !toggleBtn) return;

  const THEME_KEY = "hw_theme";
  const POPUP_KEY = "hw_theme_popup_last_open";
  ...
  function updateUI() {
    if (isDark()) {
      textEl.textContent = "Turn Lights On";
      iconEl.innerHTML = '<i class="ri-moon-line"></i>';
      iconEl.className = "text-2xl text-blue-400";
    } else {
      textEl.textContent = "Turn Lights Off";
      iconEl.innerHTML = '<i class="ri-sun-line"></i>';
      iconEl.className = "text-2xl text-yellow-500";
    }
  }
...
/* -----------------------------
   Auto popup disabled (safe)
----------------------------- */
const today = new Date().toISOString().slice(0, 10);
const lastOpen = localStorage.getItem(POPUP_KEY);

// ❌ Do NOT auto open popup
// Just keep storage logic so nothing else breaks
if (!lastOpen) {
